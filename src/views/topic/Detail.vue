...
const drsc = `1~100号利润权NFT卡发售安排：
1~10号卡（双卡共5张）：12月27日 北京时间21：00公开拍卖（竞投价上限：每张等值30000USDT）；
1~10号卡（单卡共5张）：12月28日 北京时间21：00公开拍卖（竞投价上限：每张等值30000USDT）；
11~20号卡（共10张）：元软员工（工作半年以上）及幕后长期支援元软运作的朋友；
21~70号卡（共50张）：12月26日 北京时间21：00公开售卖，每张售价500USDT；
71~100号卡（共30张）：BSC链上发售（具体时间和售价待定）

              《元灵大陆》游戏是MetaSoft元软第一个拓荒项目，有责任确保游戏最终推出，但事实上也存在各种不确定性，巨大的机会与风险并存。若以游戏利润分配权NFT卡（限区块链服务器）的方式参与《元灵大陆》，是机会亦有风险。
...
function itemBuyBtnText(metabot: GetMetaBotListResItem) {
  ...
  if (metabot.nftSellState === 3) {
    ...
    // 拍卖未开始
    return new Decimal(metabot.nftStartingPrice).div(Math.pow(10, 8)).toString() + ' BSV'
  } else if (metabot.nftSellState === 7 || metabot.nftSellState === 8) {
    // 拍卖中 || 拍卖结束
    return new Decimal(metabot.nftCurrentBidPrice).div(Math.pow(10, 8)).toString() + ' BSV'
  } else {
